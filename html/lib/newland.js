...
//分析一个场景所包含的网格、骨骼、动画、材质
newland.analyseScene=function(scene)
{
    var obj_scene={}
    var arr_mat=[];
    var len=scene.materials.length;
    for(var i=0;i<len;i++)
    {
    ...
    //对于每个网格
    var arr_mesh=[];
    var len_m=scene.meshes.length;
    for(var i=0;i<len_m;i++)
    {
        var mesh=scene.meshes[i];
        var m=null;
        if(mesh.material)//关联材质
        {
            var name_m=mesh.material.name;
            for(var j=0;j<len;j++)
            {
                var mat=arr_mat[j];
                if(mat.name==name_m)
    ...
    var arr_trans=[];
    var len_t=scene.transformNodes.length;
    for(var i=0;i<len_t;i++)
    {
        var transformNode=scene.transformNodes[i];
        arr_trans.push({
    ...
    var arr_skeletons=[];
    var len_s=scene.skeletons.length;
    for(var i=0;i<len_s;i++)
    {
        var skeleton=scene.skeletons[i];
        var arr_bone=[];
        var len2=skeleton.bones.length;
        ...
        for(var j=0;j<len2;j++)
        {
            ...
            for(var k=0;k<len3;k++)
            {
                ...
                for(var l=0;l<len2;l++)
                {
                    if(l!=j)
                    {
                        if(arr_bone[l].name==c.name)
                        {
                            bone.c.push(arr_bone[l]);
                            arr_bone[l].p=bone;
                            break;
                        }
                    }
    ...
    //排列网格与变换节点的父子关系，并关联骨骼索引
    for(var i=0;i<len_m;i++)//对于每个网格
    {
        var mesh=arr_mesh[i];
        mesh.cm=[];
        mesh.ct=[];
        if(mesh.e._children)
        {
            var len3=mesh.e._children.length;
            //对于每一个子元素
            for(var j=0;j<len3;j++)
            {
                var c=mesh.e._children[j];
                var name=c.name;
                if(c._geometry)//如果子元素是一个网格
                {
                    for(var k=0;k<len_m;k++)
                    {
                        if(k!=i)
                        {
                            var mesh2=arr_mesh[k];
                            if(mesh2.name==name)
                            {
                                mesh.cm.push(mesh2);
                                mesh2.p=mesh;
        ...
        if(mesh.e.skeleton)//如果网格有对应的骨骼系统
        {
            for(var j=0;j<len_s;j++)
            {
                if(arr_skeletons[j].name==mesh.e.skeleton.name)
                {
                    mesh.sk=arr_skeletons[j];
                    break;
                }
            }
        }
    }
    for(var i=0;i<len_t;i++)//对于每个变换节点
    {
        var node=arr_trans[i];
        node.cm=[];
        node.ct=[];
        if(node.e._children)
        {
            var len3=node.e._children.length;
            //对于每一个子元素
            for(var j=0;j<len3;j++)
            {
                var c=node.e._children[j];
                var name=c.name;
                if(!c._geometry)//如果子元素是一个变换节点
                {
                    for(var k=0;k<len_t;k++)
                    {
                        if(k!=i)
    ...
    obj_scene.arr_mat=arr_mat;
    obj_scene.arr_mesh=arr_mesh;
    obj_scene.arr_trans=arr_trans;
    obj_scene.arr_sk=arr_skeletons;
...
//将总额按随机比例分给指定对象
newland.randomDistribute=function(count,sum)
{
    var arr=[];
    var sum_rate=0;
    ...
    for(var i=0;i<count;i++)
    {
        arr[i]=(arr[i]/sum_rate)*sum;
    }
    return arr;
}
newland.s2ab=function(s) {
...
newland.saveAs=function(obj, fileName)
{
